...
export function ProjectCard(props: { card: ProjectCardType }) {
  const project: ProjectCardType = props.card;
        ...
        <div className="flex flex-row items-center gap-3 mb-2">
          ...
          {isOverflow ? (
            <div className="relative ml-auto">
              <button
                type="button"
                className={`w-7 h-7 flex font-extrabold justify-center tooltip items-center rounded-full text-sm transition duration-300 ${
                  tooltipVisible ? "bg-fg-200 text-fg-reverse" : "bg-fg-800"
                }`}
            ...
            {Object.entries(project.links).map(([key, val]) => {
              return (
                ...
                >
                  ...
                  <span className="underline-transition after:bg-white">
                    {key}
                  </span>
                </a>
              );
